...
export default function BookPageClient({ userId, initialSlots }: Props) {
  ...
  const [selectedDate, setSelectedDate] = useState<string | null>(() => {
    ...
    return formatDayKey(new Date(initialSlots[0].start_time));
  });
  ...
  const [viewMonth, setViewMonth] = useState(() => {
    if (initialSlots.length === 0) return startOfMonth(new Date());
    return startOfMonth(new Date(initialSlots[0].start_time));
  });
  const [mobileView, setMobileView] = useState<"calendar" | "info">("calendar");

  // If the page sits open for a while, the server-rendered slot list can go
  // stale (other users book some of them). Re-fetch in the background once on
  // mount so the calendar reflects current state, but don't block the UI on it.
  useEffect(() => {
    let cancelled = false;
    (async () => {
