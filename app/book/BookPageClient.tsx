...
export default function BookPageClient({ initialSlots }: Props) {
  const router = useRouter();
  const { data: slots = [], mutate } = useAvailableSlots(initialSlots);
  ...
  async function confirmBooking() {
    ...
    if (!slotCheck || slotCheck.is_booked) {
      ...
      setSelected(null);
      mutate(
        (current?: Slot[]) => current?.filter((s) => s.id !== selected.id) ?? [],
        { revalidate: true }
      );
      return;
    }
